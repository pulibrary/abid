...
# ActiveSupport `blank?` / `present?` are gone; the inline checks below
# reproduce its String semantics exactly (nil or all-whitespace is blank),
# using the same /\A[[:space:]]*\z/ test ActiveSupport used.
module Aspace
  class Client < ArchivesSpace::Client
    ...
    def find_resource_uri(ead_id:)
      return if ead_id.nil? || /\A[[:space:]]*\z/.match?(ead_id.to_s)
      repositories.each do |repository|
        ...
        return uri unless uri.nil? || /\A[[:space:]]*\z/.match?(uri.to_s)
      end
    end
