...
class Batch < ApplicationRecord
  ...
  # The scope lambda takes the relation as an argument, which is how
  # ApplicationRecord calls it. `order("created_at")` is spelled with a symbol
  # so Sequel orders by the column rather than by a string literal.
  has_many :absolute_identifiers, ->(scope) { scope.order(:created_at) }, dependent: :destroy, as: :batch
  belongs_to :user
  ...
  def call_number_exists_in_aspace
    ...
    return unless value_blank?(resource_uri)
    resource_uri = aspace_client.find_resource_uri(ead_id: call_number)
    if value_blank?(resource_uri)
  ...
  def ready_for_aspace_checks?
    !value_blank?(resource_uri) && !value_blank?(start_box) && !value_blank?(end_box) && end_box >= start_box
  end
  ...
  def repository_uri
    return if value_blank?(resource_uri)
