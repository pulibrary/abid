...
class AbsoluteIdentifier < ApplicationRecord
  ...
  def full_identifier
    return if value_blank?(suffix)
    format("#{prefix}-%.6d", suffix)
  end

  def set_suffix
    return if !value_blank?(suffix) || !generate_abid
  ...
  def alma_item
    @alma_item ||=
      begin
        return Alma::BibItem.new(holding_cache) unless value_blank?(holding_cache)
        item = Alma::BibItem.find_by_barcode(barcode)
        if value_blank?(item.item.dig("errorList", "error", 0, "errorCode"))
          item
        end
      end
  end
  ...
  def previous_call_number
    return if value_blank?(holding_cache)
    alma_item.holding_data["permanent_call_number"]
  end

  def cache_holding_id
    return if !value_blank?(holding_id) || !batch.is_a?(MarcBatch) || value_blank?(alma_item)
  ...
  def barcode_in_alma
    return unless batch.is_a?(MarcBatch)
    return unless value_blank?(alma_item)
  ...
  def holding_id_unique
    ...
    return if value_blank?(existing_abids)
